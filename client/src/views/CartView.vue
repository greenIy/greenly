    ...
    <div class="d-flex justify-content-center my-2 mx-4 card-general">
      <div class="card mt-5">
        <div class="card-body w-100">
          <h2 class="page-title text-start align-middle mt-auto">
            Carrinho de Compras
          </h2>
          <div v-if="this.cartLength > 0" class="text-start mb-2">
            <font-awesome-icon :icon="['fa', 'sun']" style="color: orange" />
            Recursos renováveis fornecedor |
            <font-awesome-icon
              :icon="['fa', 'warehouse']"
              style="color: grey"
            />
            Média recursos fornecedor |
            <font-awesome-icon :icon="['fa', 'gas-pump']" style="color: " />
            Média recursos transportador |
            <font-awesome-icon
              :icon="['fa', 'skull-crossbones']"
              style="color: red"
            />
            Média emissões transportador
          </div>
          <div>
            <div
              v-if="this.cartLength == 0"
              style="overflow-y: hidden; overflow-x: auto; height: 370px"
              class="content d-flex mb-3"
            >
              <div class="w-100 justify-content-center p-5 mt-5">
                <p class="text-center">
                  Parece que ainda não adicionou nenhum produto ao carrinho.<br />Do
            ...
            <div v-else class="row gx-2">
              <div class="card p-3 container" style="height: 40vh">
                <div class="row title">
                  ...
                  <div class="col-2">Informação Ambiental</div>
                  <div class="col-1">Preço</div>
                  <div class="col-2">Preço transporte</div>
      ...
      <div v-if="this.cartLength != 0" class="d-flex ms-2">
        <div class="card p-3 my-5 w-100">
              ...
              <tbody>
                <tr class="align-middle">
                  <td style="font-size: 15px">
                    Percentagem de recursos renováveis dos fornecedores
                  </td>
                  <td style="font-size: 13px">
                    {{ this.totalSupplierRenewableResources }}%
                  </td>
                </tr>
                <tr class="align-middle">
                  <td style="font-size: 15px">
                    Média de consumos dos fornecedores
                  </td>
                  <td style="font-size: 13px">
                    {{ this.totalSupplierResourceUsage }} kWh
                  </td>
                </tr>
                <tr class="align-middle">
                  <td style="font-size: 15px">
                    Média de consumos dos transportadores
                  </td>
                  <td style="font-size: 13px">
                    {{ this.totalTransporterResourceUsage }} l/100Km
                  </td>
                </tr>
                <tr class="align-middle">
                  <td style="font-size: 15px">
                    Média de emissões poluentes dos transportadores
                  </td>
                  <td style="font-size: 13px">
                    {{ this.totalTransporterEmissions }} CO2 g/Km/t
                  </td>
                </tr>
          ...
          <div
            class="d-flex align-items-end justify-content-end h-100 me-3 mb-3"
          >
        ...
        http
          ...
          .then((response) => {
            if (response.status == 200) {
              ...
              this.totalSupplierRenewableResources =
                response.data.total_supplier_renewable_resources;
              this.totalSupplierResourceUsage =
                response.data.total_supplier_resource_usage;
              this.totalTransporterResourceUsage =
                response.data.total_transporter_resource_usage;
              this.totalTransporterEmissions =
                response.data.total_transporter_emissions;
