<template>
  <div class="container my-2 d-flex mainContainer">
      ...
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="newCenterModalLabel">
          ...
          <form @submit.prevent="newDistributionCenter()">
            <div class="modal-body">
              <div class="col mb-3">
                <label for="newCenterCapacity" class="form-label"
                  >Capacidade em m² <span style="color: #ff0000">*</span></label
                >
                <input
                  type="number"
                  class="form-control w-50 ms-3"
                  id="newCenterCapacity"
                  v-model="this.newCenterInfo.capacity"
                  placeholder="Capacidade"
                  required
                />
              </div>
              <div class="col mb-3">
                <label for="newCenterAddress" class="form-label"
                  >Morada <span style="color: #ff0000">*</span></label
                ><br />
                <div
                  v-if="!this.newCenterInfo.address.street"
                  class="mb-3 ms-3"
                >
                    ...
                    <div class="card-body">
                      <address>
                        <strong
                          ...
                          />&nbsp; {{ this.newCenterInfo.address.city }}</strong
                        ><br />
                        {{ this.newCenterInfo.address.street }}<br />
                        {{ this.newCenterInfo.address.city }},
                        {{ this.newCenterInfo.address.country }}<br />
                        Código Postal:
                        {{ this.newCenterInfo.address.postal_code }}<br />
                ...
                <button
                  ...
                  data-bs-toggle="modal"
                  data-bs-target="#chooseAddressNewCenter"
                  id="newCenterAddress"
                  v-on:click=""
                >
    ...
    <div
      class="modal fade"
      id="chooseAddressNewCenter"
      data-bs-backdrop="static"
      data-bs-keyboard="false"
      tabindex="-1"
      aria-labelledby="chooseAddressNewCenterLabel"
      aria-hidden="true"
    >
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="chooseAddressNewCenterLabel">
              Selecione a morada do centro de distribuição
            </h5>
            <button
              type="button"
              class="btn-close"
          ...
          <div class="modal-body">
            ...
            >
              <div
                ...
                style="width: 300px !important; cursor: pointer"
                v-on:click="this.newCenterInfo.address = address"
                data-bs-toggle="modal"
                data-bs-target="#newCenterModal"
              >
                <div class="card-body">
    ...
    <div
      class="modal fade"
      id="detailsCenterModal"
      data-bs-backdrop="static"
      data-bs-keyboard="false"
      tabindex="-1"
      aria-labelledby="detailsCenterModalLabel"
      aria-hidden="true"
    >
      <div class="modal-dialog modal-lg modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="detailsCenterModalLabel">
              Centro de distribuição #{{ this.selectedCenter.id }}
            </h5>
            <button
              type="button"
              class="btn-close"
          ...
          <div class="modal-body">
              ...
              <div class="row align-items-start">
                <div class="col">
                  ...
                  <address class="ms-2 mt-2">
                    ...
                    {{ this.selectedCenter.address.country }}<br />
                    <abbr title="CP">Código Postal:</abbr>
                    {{ this.selectedCenter.address.postal_code }}
                  </address>
            ...
            <div class="container">
              ...
              <p class="mt-3 ms-2 fs-5">
                Dimensão:
                {{ Math.ceil(this.selectedCenter.total_vehicles * 36) }}m²/{{
                  this.selectedCenter.capacity
                }}m²
              </p>
              <br />
              <div
                class="row align-items-start"
                style="max-height: 210px; min-height: 185px; overflow-y: auto"
              >
                <div
                  v-if="getCenterTotalVehicles(this.selectedCenter) > 0"
                  v-for="(vehicle, index) in this.selectedCenter.vehicles"
                  :key="vehicle.id"
                  class="card ms-2 me-4"
                  style="width: 20.5%; height: 150px !important"
                >
                  <div class="card-body">
                ...
                <div v-else class="text-center fs-5 mt-5">
                  <p>
                    Parece que ainda não adicionou veículos a este centro de
                    distribuição.
                  </p>
                </div>
              </div>
              <div class="text-left mt-4">
                <div class="progress me-3" style="height: 28px">
                  <div
                    id="selectedCapacityBar"
                    class="progress-bar"
                    role="progressbar"
                    aria-valuemin="0"
                    aria-valuemax="100"
                  >
                    {{ this.selectedCenter.total_vehicles }} /
                    {{ Math.floor(this.selectedCenter.capacity / 36) }}
                  </div>
                </div>
              </div>
              <div class="text-center">
        ...
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="chooseAddressModalLabel">
              Selecione morada do centro #{{ this.selectedCenter.id }}
            </h5>
            <button
              type="button"
              class="btn-close"
          ...
          <div class="modal-body">
            ...
            >
              ...
              >
                <div
                  class="card-body"
                  v-on:click="editCenterAddress(this.selectedCenter, address)"
                  data-bs-toggle="modal"
                  data-bs-target="#detailsCenterModal"
                >
                  <address>
              ...
              <div v-else class="text-center mt-3 mb-3">
                <p>
                  Parece que não se encontram moradas disponíveis.
                  <br />Por favor adicione uma ao seu
                  <router-link
                    class="greenly-link"
                    to="/perfil/moradas"
                    data-bs-dismiss="modal"
    ...
    >
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="changeCapacityModalLabel">
              Centro de distribuição #{{ this.selectedCenter.id }}
            </h5>
            <button
              type="button"
    ...
    <div
      ...
      data-bs-keyboard="false"
      tabindex="-1"
      aria-labelledby="removeCenterLabel"
      aria-hidden="true"
    >
      <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="removeCenterLabel">
              Remover centro de distribuição #{{ this.selectedCenter.id }}
            </h5>
            <button
              type="button"
              class="btn-close"
          ...
          <div class="modal-body">
            <p>
              Tem a certeza que pretende remover este centro de distribuição?
            </p>
          </div>
          <div class="modal-footer">
            <button
              type="button"
              id="closeRemoveCenter"
              class="btn btn-secondary"
              data-bs-dismiss="modal"
            >
              Cancelar
            ...
            <button
              ...
              v-on:click="removeDistributionCenter(this.selectedCenter.id)"
            >
              Remover
            </button>
          </div>
...
export default {
  ...
  methods: {
    ...
    initCenterMap() {
      ...
      loader.load().then(() => {
        let position = {
          lat: this.selectedCenter.address.latitude,
          lng: this.selectedCenter.address.longitude,
        };
        let centerMap = new google.maps.Map(
          document.getElementById("centerMap"),
          {
            center: position,
            zoom: 12,
            streetViewControl: false,
    ...
    getDistributionCenterInfo(center) {
      ...
      if (accessToken && userId) {
        http
          .get(`/transporter/${userId}/centers/${center.id}`, headers)
          .then((response) => {
            if (response.status == 200) {
    ...
    calculateDistributionCentersLength() {
      this.distributionCentersLength = Object.keys(
    ...
    getCenterCapacity(center) {
      let vehicleEstimate =
        Math.floor(center.capacity / 36) > 0
          ? Math.floor(center.capacity / 36)
          : 1;
    ...
    getDistributionsCenterCapacity(index) {
      ...
      let vehicleEstimate =
        Math.floor(distributionCenter.capacity / 36) > 0
          ? Math.floor(distributionCenter.capacity / 36)
          : 1;
      return (distributionCenter.total_vehicles / vehicleEstimate) * 100;
    },
    getUnusedAddresses() {
      return this.user.addresses.filter((address) => {
        let usedDistributionCenterAddresses = this.distributionCenters.map(
          (center) => center.address.id
        );
    ...
    successfulNewCenter() {
      ...
      this.successfulToast(
        "Adicionado! O novo centro de distribuição foi adicionado com sucesso."
      );
    },
    successfulNewCapacity() {
      ...
      this.successfulToast(
        "Alterada! A capacidade máxima do centro foi alterada com sucesso."
      );
    },
    successfulRemoveCenter() {
      ...
      closeEditModal.click();
      this.successfulToast(
        "Removido! O centro de distribuição foi removido com sucesso."
      );
    },
    ...
    editCenterAddress(center, address) {
      ...
      if (accessToken && userId) {
        http
          .put(
            `/transporter/${userId}/centers/${center.id}`,
            JSON.stringify({
              address: address.id,
            }),
            headers
          ...
          .then((response) => {
            if (response.status == 201) {
              ...
              this.getUserDistributionCenters();
              this.successfulToast(
                "Alterada! A morada do centro foi alterada com sucesso."
              );
            }
          })
          .catch((error) => {
    ...
    editCenterCapacity(center) {
      ...
      let newCapacity = document.getElementById(
        "newSelectedCenterCapacity"
      ).value;
      if (accessToken && userId) {
        http
          .put(
    ...
    removeDistributionCenter(center) {
      ...
      if (accessToken && userId) {
        http
          .delete(`/transporter/${userId}/centers/${center}`, headers)
          .then((response) => {
