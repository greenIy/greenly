...
<div class="my-5">
  ...
  <hr />
  <i
    >Todas as suas encomendas encontrar-se-ão nesta secção, ordenadas
    cronologicamente.<br />É possivel cancelar individualmente um item de
    qualquer encomenda caso este ainda não se encontre em trânsito
  ...
  <button
    type="button"
    class="btn btn-secondary btn-sm btnJson"
    v-on:click="saveFile()"
  >
    <font-awesome-icon :icon="['fa', 'download']" /> &nbsp;Descarregar dados
    (JSON)
  </button>
  <div
    v-if="this.ordersLength == 0"
    style="overflow-x: hidden; overflow-y: auto; height: 370px"
  >
    <div class="w-100 justify-content-center p-5 mt-3">
      <p class="text-center">
        Parece que ainda não realizou nenhuma encomenda.<br />Do que está à
        espera? Explore o nosso catálogo.
      </p>
  ...
  <div v-else style="overflow-x: hidden; overflow-y: auto; height: 370px">
    ...
    >
                ...
                <div class="col">
                  <font-awesome-icon :icon="['fa', 'calendar']" />&nbsp;{{
                    `${String(new Date(order.date).getDate()).padStart(
                      2,
                      "0"
                    )}/${String(new Date(order.date).getMonth() + 1).padStart(
                      2,
                      "0"
              ...
              <div class="container mt-2" style="width: 60%">
                <div v-for="item in order.items" :key="item.id">
                  <div class="row">
                    <div class="col">
                      <h5>
                        ...
                        <router-link
                          class="greenly-link"
                          :to="{ path: '/produto/' + item.product.id }"
                        >
                          <font-awesome-icon
                      ...
                      >
                        <span
                          v-if="
                            ...
                            item.status == 'AWAITING_TRANSPORT' ||
                            item.status == 'TRANSPORT_IMMINENT'
                          "
                          role="button"
                          style="color: #d21d1d"
                          v-on:click="
                            this.selectOrder(order), this.selectProduct(item)
                          "
                          data-bs-toggle="modal"
                          data-bs-target="#cancelProduct"
                  ...
                  <div class="row mt-1">
                    ...
                    <div class="col">Quantidade: {{ item.quantity }}</div>
                  </div>
                  <div class="row mt-1">
                    <div class="col">Fornecedor: {{ item.supplier.name }}</div>
                    <div class="col">
                    ...
                    <div class="col">
                      <span v-if="item.status == 'COMPLETE'"
                        ...
                        <b style="color: #198754">{{
                          this.getProductStatus(item.status)
                        }}</b></span
                      >
                      <span
                        v-else-if="
                          item.status == 'FAILURE' || item.status == 'CANCELED'
                        "
                        >Estado:
                        <b style="color: #d21d1d">{{
                          this.getProductStatus(item.status)
                        }}</b></span
                      >
                      <span v-else
                        >Estado: {{ this.getProductStatus(item.status) }}</span
                      >
                    </div>
                  </div>
      ...
      <div
        ...
        tabindex="-1"
        aria-labelledby="cancelProductLabel"
        aria-hidden="true"
      >
        <div class="modal-dialog modal-dialog-centered">
          <div class="modal-content">
            ...
            <div class="modal-body">
              Esta ação é irreversível. Tem a certeza que deseja cancelar este
              item?
            </div>
            <div class="modal-footer">
              <button
                type="button"
                id="closeCancelProductModal"
              ...
              <button
                ...
                v-on:click="
                  cancelProduct(this.selectedOrder.id, this.selectedProduct.id)
                "
              >
                Sim
              </button>
  ...
  methods: {
    ...
    getUserOrders() {
      ...
      if (accessToken && userId) {
        http
          .get(`/user/${userId}/orders`, headers)
          .then((response) => {
    ...
    getOrderStatus(orderIndex) {
      ...
      for (
        let productIndex = 0;
        productIndex < Object.keys(this.orders[orderIndex].items).length;
        productIndex++
      ) {
      ...
      if (Object.keys(this.orders[orderIndex].items).length == waitingProduct) {
        ...
        Object.keys(this.orders[orderIndex].items).length == completeProduct
      ) {
        return 2;
      } else if (
        Object.keys(this.orders[orderIndex].items).length == failureProduct
      ) {
        return 3;
      } else if (
        Object.keys(this.orders[orderIndex].items).length == canceledProduct
      ) {
        return 4;
      } else {
        return 1;
    ...
    calculateOrderProducts(orderIndex) {
      return this.orders[orderIndex].items.reduce(
    ...
    calculateOrderDetails(orderIndex, detail) {
      ...
      if (detail === "renewable") {
        return totalRenewableResources;
      } else if (detail === "emissions") {
        return totalTransportEmissions.toFixed(2);
      } else if (detail === "supplier") {
        return totalSupplierResources.toFixed(2);
      } else if (detail === "transporter") {
        return totalTransporterResources.toFixed(2);
      }
    },
    selectOrder(order) {
    ...
    getProductStatus(state) {
      ...
      } else if (state === "AWAITING_TRANSPORT") {
        return "A aguardar transporte";
      } else if (state === "TRANSPORT_IMMINENT") {
        return "Transporte iminente";
      } else if (state === "IN_TRANSIT") {
        return "Em trânsito";
      } else if (state === "LAST_MILE") {
        return "Último quilômetro";
    ...
    successfulCancelProduct() {
      var closeCancelProductModal = document.getElementById(
        "closeCancelProductModal"
      );
      closeCancelProductModal.click();
      this.toast.success(
        "Cancelada! A entrega do item foi cancelada com sucesso.",
        {
          position: "top-right",
          timeout: 5000,
          closeOnClick: true,
    ...
    cancelProduct(order_id, product_id) {
      ...
      if (accessToken && userId) {
        http
          .put(
            `/store/orders/${order_id}/${product_id}`,
            JSON.stringify({
              status: "CANCELED",
            }),
            headers
